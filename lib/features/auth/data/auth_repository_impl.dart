...
class AuthRepositoryImpl implements AuthRepository {
  ...
  Future<UserModel> login(String email, String password) async {
    ...
    // After saving the new access token, fetch the complete
    // authenticated user's profile.
    final meResponse = await api.getMe();
    final user = UserModel.fromJson(meResponse.data['data']);
  ...
  Future<void> verifyEmail({required String userId, required String code}) {
    return api.verifyEmail(userId: userId, code: code);
  }
  ...
  @override
  Future<Map<String, dynamic>> sendEmailChangeCode({
    required String email,
  }) async {
    final response = await api.sendEmailChangeCode(email: email);
  ...
  Future<UserModel> verifyEmailChange({
    ...
    await api.verifyEmailChange(email: email, code: code);
  ...
  Future<Map<String, dynamic>> exportMyData() async {
    final response = await api.exportMyData();
    final raw = response.data;
    ...
    throw StateError('Unexpected account export response.');
  }
  ...
  Future<Map<String, dynamic>> deleteAccount({
    ...
    throw StateError('Unexpected account deletion response.');
  }
  ...
  Future<UserModel> verify2FA({
    ...
    // The 2FA verification response only contains basic identity fields.
    // After storing the new authenticated session, fetch the complete user.
    final meResponse = await api.getMe();
    final user = UserModel.fromJson(meResponse.data['data']);
