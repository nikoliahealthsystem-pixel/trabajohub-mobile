...
class BillingRepositoryImpl implements BillingRepository {
  ...
  Future<WalletModel> getWallet() async {
    ...
    final wallet = data is Map<String, dynamic>
  ...
  Future<({List<PayoutModel> payouts, int total, bool hasMore})> getPayouts({
    ...
    final cached = _cache
        .get<({List<PayoutModel> payouts, int total, bool hasMore})>(key);

    if (cached != null && !cached.isStale) {
      return cached.data;
    }

    final raw = await _api.fetchPayouts(
      page: page,
      limit: limit,
      status: status,
    );
    ...
    final result = (
      payouts: data
          .map(
            (item) =>
                PayoutModel.fromJson(Map<String, dynamic>.from(item as Map)),
          )
          .toList(),
